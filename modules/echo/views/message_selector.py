...
class EchoMessageSelect(Select):
    ...
    def __init__(
        self, echo_messages: List[EchoMessage], new_content: str, new_is_embed: bool, guild
    ):
        ...
        try:
            ...
            if not selected_message:
                await interaction.response.send_message(
                    "❌ No se pudo encontrar el mensaje seleccionado.", ephemeral=True
                )
                return
            ...
            try:
                if self.new_is_embed:
                    ...
                    try:
                        ...
                        await interaction.response.send_message(
                            "❌ El JSON del embed no es válido.", ephemeral=True
                        )
                        return
                    except Exception as e:
                        await interaction.response.send_message(
                            f"❌ Error al crear el embed: {str(e)}", ephemeral=True
                        )
                        return
                else:
                ...
                await interaction.response.send_message(
                    f"✅ Mensaje editado correctamente en {channel.mention}", ephemeral=True
                )
                ...
                await interaction.response.send_message(
                    f"❌ Error al editar el mensaje: {str(e)}", ephemeral=True
                )
