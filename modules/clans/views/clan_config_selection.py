...
class ClanConfigSelectionView(View):
    ...
    async def execute_config(self, interaction: Interaction):
        ...
        try:
            ...
            embed = Embed(title=f"🔧 Configurando clan: {clan.name}", color=Color.blue())
            ...
            error = await self.service.update_clan_config(
                clan.id, max_text_channels=max_canales_texto, max_voice_channels=max_canales_voz
            )
            ...
            if max_canales_texto is not None:
                updates.append(
                    f"💬 Máximo canales texto: {clan.max_text_channels} → **{max_canales_texto}**"
                )

            if max_canales_voz is not None:
                updates.append(
                    f"🔊 Máximo canales voz: {clan.max_voice_channels} → **{max_canales_voz}**"
                )

            if updates:
                embed.add_field(
                    name="✅ **Cambios Aplicados**", value="\n".join(updates), inline=False
                )
            ...
            await interaction.followup.send(
                f"❌ Error al configurar el clan: {str(e)}", ephemeral=True
            )
