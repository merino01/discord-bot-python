...
class AutomaticMessagesScheduler:
    ...
    def _setup_scheduled_jobs(self):
        ...
        logger.info(
            "Cargando mensajes automáticos - Error: %s, Total mensajes: %s",
            error,
        ...
        if error or not messages:
            logger.warning(
                "No se pudieron cargar mensajes automáticos: %s", error or "Sin mensajes"
            )
            return
        ...
        for message in messages:
            try:
                logger.info(
                    "Procesando mensaje: ID=%s, Tipo=%s, Canal=%s, Hora=%s:%s, Días=%s",
                    message.id,
    ...
    def _schedule_interval_message(self, message: AutomaticMessage):
        ...
        def job_func():
            if self.bot.loop and not self.bot.loop.is_closed():
                asyncio.run_coroutine_threadsafe(self._send_message_safe(message), self.bot.loop)
            else:
        ...
        logger.debug(
            "Programado mensaje intervalo %s cada %d %s",
            message.id,
            message.interval,
    ...
    def _schedule_daily_message(self, message: AutomaticMessage):
        ...
        logger.info(
            "✅ Programado mensaje diario %s para las %s - Canal ID: %s",
            message.id,
            time_str,
            message.channel_id,
        )
    ...
    def _schedule_weekly_message(self, message: AutomaticMessage):
        ...
        for weekday in weekdays:
            if 0 <= weekday <= 6:
                ...
                logger.info(
                    "✅ Programado mensaje semanal %s para %s a las %s",
                    message.id,
                    day_name,
                    time_str,
                )
    ...
    def add_message_schedule(self, message: AutomaticMessage):
        ...
        try:
            logger.info(
                "Agregando programación para mensaje: ID=%s, Tipo=%s",
                message.id,
                message.schedule_type,
            )
            self._schedule_message(message)
    ...
    async def _send_message(self, message: AutomaticMessage):
        ...
        try:
            ...
            if not channel:
                logger.warning(
                    "Canal %s no encontrado para mensaje %s", message.channel_id, message.id
                )
                return
            ...
            if sent_message:
                ...
                logger.info(
                    "Mensaje automático enviado: %s en canal %s", message.display_name, channel.name
                )
            else:
                logger.error("No se pudo enviar el mensaje %s", message.id)

        except Exception as e:
            logger.error("Error enviando mensaje automático %s: %s", message.id, str(e))
    ...
    async def send_category_message(self, category_id: int, new_channel):
        ...
        try:
            ...
            logger.error("Error enviando mensajes de categoría %s: %s", category_id, str(e))
