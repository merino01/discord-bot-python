...
class MessageSelectView(View):
    ...
    def _get_message_description(self, message: AutomaticMessage) -> str:
        ...
        if message.channel_id:
            ...
            target = (
                f"{constants.CATEGORY_PREFIX}{category.name}"
                if category
                else constants.CATEGORY_UNKNOWN
            )
...
class MessageSelect(Select):
    ...
    def __init__(self, options: List[SelectOption], action: str, bot):
        placeholder = (
            constants.SELECT_PLACEHOLDER_DELETE
            if action == "delete"
    ...
    async def callback(self, interaction: Interaction):
        ...
        if error or not message:
            await send_error_message(
                interaction, constants.ERROR_MESSAGE_NOT_FOUND.format(id=message_id)
            )
            return
...
class ConfirmDeleteView(View):
    ...
    @discord.ui.button(
        label=constants.BUTTON_CONFIRM_DELETE, style=discord.ButtonStyle.danger, emoji="✅"
    )
    ...
    @discord.ui.button(
        label=constants.BUTTON_CANCEL_DELETE, style=discord.ButtonStyle.secondary, emoji="❌"
    )
