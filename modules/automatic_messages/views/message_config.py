...
class ProgramMessageTextModal(Modal):
    ...
    async def on_submit(self, interaction: Interaction):
        try:
            ...
            await interaction.response.send_message(
                constants.TEXT_ERROR_PREFIX.format(error=str(e)), ephemeral=True
            )
    ...
    async def _show_message_config_options(self, interaction: Interaction, text: str):
        ...
        embed = discord.Embed(
            ...
            description=constants.DESC_MESSAGE_PREVIEW.format(
                preview=(
                    text[:200] + (constants.TEXT_CONTINUE_ELLIPSIS if len(text) > 200 else '')
                    if text
                    else constants.TEXT_NO_TEXT_ONLY_EMBED
                )
            )
            + "\n\n"
...
class MessageConfigOptionsView(View):
    ...
    @discord.ui.button(
        label=constants.BUTTON_SIMPLE_MESSAGE, style=discord.ButtonStyle.secondary, emoji="📝"
    )
    ...
    @discord.ui.button(
        label=constants.BUTTON_CREATE_EMBED, style=discord.ButtonStyle.primary, emoji="🎨"
    )
    ...
    @discord.ui.button(
        label=constants.BUTTON_ADD_IMAGE_BTN, style=discord.ButtonStyle.primary, emoji="🖼️"
    )
    ...
    @discord.ui.button(
        label=constants.BUTTON_EMBED_AND_IMAGE, style=discord.ButtonStyle.primary, emoji="✨"
    )
        ...
        if message_data['schedule_type'] == "on_channel_create":
            ...
            embed = discord.Embed(
                ...
                description=constants.DESC_SCHEDULE_CONFIG.format(
                    preview=message_data['text'][:200]
                    + (constants.TEXT_CONTINUE_ELLIPSIS if len(message_data['text']) > 200 else '')
                ),
                color=discord.Color.blue(),
            )
