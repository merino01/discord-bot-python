...
class ProgramMessageModal(Modal):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        await interaction.response.send_message(constants.TEXT_MODAL_COMPLETED, ephemeral=True)
...
class ScheduleConfigView(View):
    ...
    def _add_config_buttons(self, schedule_type: str):
        ...
        if schedule_type == "interval":
            ...
            button = discord.ui.Button(
                label=constants.BUTTON_CONFIGURE_HOUR, style=discord.ButtonStyle.primary, emoji="🕐"
            )
    ...
    async def _configure_time(self, interaction: Interaction):
        ...
        title = (
            constants.TITLE_DAILY_HOUR_CONFIG
            if schedule_type == "daily"
            else constants.TITLE_WEEKLY_CONFIG
        )
        description = (
            constants.DESC_DAILY_CONFIG
            if schedule_type == "daily"
            else constants.DESC_WEEKLY_CONFIG
        )
