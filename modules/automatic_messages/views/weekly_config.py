...
class WeekdaySelectionView(View):
    ...
    async def confirm_button(self, interaction: Interaction, button: discord.ui.Button):
        if not self.selected_weekdays:
            await send_error_message(interaction, constants.ERROR_SELECT_ONE_DAY)
            return
    ...
    async def _create_weekly_message(self, interaction: Interaction):
        ...
        try:
            ...
            weekday_names = [
                constants.WEEKDAY_TRANSLATIONS[day] for day in sorted(self.selected_weekdays)
            ]
...
class WeekdayMultiSelect(Select):
    ...
    def __init__(self):
        options = [
            SelectOption(label=constants.WEEKDAY_MONDAY, value="0", emoji="📅"),
            SelectOption(label=constants.WEEKDAY_TUESDAY, value="1", emoji="📅"),
            SelectOption(label=constants.WEEKDAY_WEDNESDAY, value="2", emoji="📅"),
            SelectOption(label=constants.WEEKDAY_THURSDAY, value="3", emoji="📅"),
            SelectOption(label=constants.WEEKDAY_FRIDAY, value="4", emoji="📅"),
            SelectOption(label=constants.WEEKDAY_SATURDAY, value="5", emoji="📅"),
            SelectOption(label=constants.WEEKDAY_SUNDAY, value="6", emoji="📅"),
        ]
