...
from discord import app_commands, Interaction, TextChannel, CategoryChannel, Embed, Color, Object
from discord.ext import commands
from settings import guild_id
from .services import AutomaticMessagesService
from .utils import validate_cron_expression, validate_time, send_error_message
from .views import MessageSelectView, MessageBuilderView
...
class AutomaticMessagesCommands(commands.GroupCog, name="mensajes_automaticos"):
    ...
    async def program_message(
        ...
        if not isinstance(destino, (TextChannel, CategoryChannel)):
            await send_error_message(interaction, constants.ERROR_INVALID_DESTINATION)
            return
        ...
        if tipo_programacion == "on_channel_create" and not isinstance(destino, CategoryChannel):
            await send_error_message(interaction, constants.ERROR_CATEGORY_REQUIRED)
            return
    ...
    @app_commands.describe(
        canal="Filtrar por canal específico", categoria="Filtrar por categoría específica"
    )
    @app_commands.checks.has_permissions(manage_channels=True, manage_messages=True)
    ...
    async def on_app_command_error(
        ...
        if isinstance(error, app_commands.MissingPermissions):
            ...
            await send_error_message(interaction, constants.TEXT_UNEXPECTED_ERROR)
