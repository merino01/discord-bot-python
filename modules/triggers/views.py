...
class TriggerSelectView(ui.View):
    ...
    def __init__(
        ...
        callback_func: Callable[[Interaction, str], Any],
        base_title: str,
        timeout: float = 180,
    ):
    ...
    def _update_buttons(self):
        ...
        for i, trigger in enumerate(page_triggers):
            button = TriggerButton(
                trigger=trigger, callback_func=self.callback_func, row=i // 5  # 5 botones por fila
            )
        ...
        if self.max_pages > 1:
            ...
            if self.current_page > 0:
                prev_button = ui.Button(
                    label=constants.VIEW_BUTTON_PREVIOUS, style=ButtonStyle.primary, row=4
                )
            ...
            page_indicator = ui.Button(
                label=constants.PAGE_INDICATOR.format(
                    current=self.current_page + 1, total=self.max_pages
                ),
            ...
            if self.current_page < self.max_pages - 1:
                next_button = ui.Button(
                    label=constants.VIEW_BUTTON_NEXT, style=ButtonStyle.primary, row=4
                )
    ...
    async def _prev_page(self, interaction: Interaction):
        ...
        if self.current_page > 0:
            ...
            title = constants.VIEW_PAGE_TITLE.format(
                action=self.base_title, current=self.current_page + 1, total=self.max_pages
            )
            embed = create_trigger_selection_embed(
                self.triggers, title, self.current_page, self.triggers_per_page
            )
...
class TriggerButton(ui.Button):
    ...
    def __init__(
        self, trigger: Trigger, callback_func: Callable[[Interaction, str], Any], row: int
    ):
...
def create_trigger_selection_embed(
    triggers: List[Trigger], title: str, current_page: int = 0, triggers_per_page: int = 20
) -> Embed:
    """Crear embed para mostrar la lista de triggers disponibles con paginación"""
    embed = Embed(title=title, description=constants.VIEW_SELECTION_DESCRIPTION, color=Color.blue())
    ...
    embed.add_field(
        ...
        value=constants.VIEW_SUMMARY_TEXT.format(total=total_triggers, in_page=triggers_in_page),
        inline=True,
    )
    ...
    if max_pages > 1:
        embed.add_field(
            ...
            value=constants.PAGINATION_INFO.format(
                current=current_page + 1, total=max_pages, count=total_triggers
            ),
            inline=True,
        )

    embed.add_field(
        name=constants.VIEW_INSTRUCTIONS_TITLE, value=constants.VIEW_INSTRUCTIONS_TEXT, inline=False
    )
