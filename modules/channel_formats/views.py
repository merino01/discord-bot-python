...
class ChannelFormatSelectView(discord.ui.View):
    ...
    def __init__(
        self,
        channel_formats: List[ChannelFormat],
        callback: Callable,
        page: int = 0,
        per_page: int = 20,
    ):
    ...
    def add_navigation_buttons(self):
        ...
        if self.page > 0:
            prev_button = discord.ui.Button(
                label=constants.PAGE_PREVIOUS, style=discord.ButtonStyle.secondary, emoji="⬅️"
            )
        ...
        if self.page < self.total_pages - 1:
            next_button = discord.ui.Button(
                label=constants.PAGE_NEXT, style=discord.ButtonStyle.secondary, emoji="➡️"
            )
    ...
    async def previous_page(self, interaction: Interaction):
        ...
        if self.page > 0:
            new_view = ChannelFormatSelectView(
                self.channel_formats, self.callback_func, self.page - 1, self.per_page
            )
            embed = create_channel_format_selection_embed(
                self.channel_formats, self.page - 1, self.per_page
            )
...
class ChannelFormatButton(discord.ui.Button):
    ...
    def __init__(self, channel_format: ChannelFormat, callback: Callable):
        ...
        super().__init__(
            label=label, style=discord.ButtonStyle.primary, custom_id=channel_format.id
        )
...
def create_channel_format_selection_embed(
    channel_formats: List[ChannelFormat], page: int = 0, per_page: int = 20
) -> Embed:
    ...
    if total_pages > 1:
        embed.set_footer(
            text=constants.PAGE_INFO.format(
                current=page + 1,
                total=total_pages,
