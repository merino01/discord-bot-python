...
class ChannelFormatsCommands(commands.GroupCog, name="formato_canales"):
    ...
    async def list_channel_formats(
        ...
        if id_formato:
            ...
            channel_formats, error = ChannelFormatsService.get_all_by_channel_id(canal.id)
            if error:
                await interaction.response.send_message(
                    content=error,
                    ephemeral=True
    ...
    async def delete_channel_format(
        ...
        if error:
            ...
            return
        await interaction.response.send_message(
            f"Formato de canal eliminado: {channel_format.id}",
            ephemeral=True
        )
    ...
    @app_commands.command(
        name="editar",
        description="Edita un formato de canal"
    )
    @app_commands.describe(
        id_formato="ID del formato de canal",
        canal="Canal donde se aplicará el formato",
