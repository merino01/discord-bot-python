...
class ChannelFormatsCommands(commands.GroupCog, name="formato_canales"):
    ...
    async def list_channel_formats(
        ...
        for channel_format in channel_formats:
            embed = Embed(title=constants.TITLE_FORMAT_ID.format(id=channel_format.id), color=Color.blue())
            embed.add_field(name=constants.FIELD_CHANNEL, value=f"<#{channel_format.channel_id}>", inline=True)
